...
def build_model(x_train, y_train, x_validation, y_validation, x_test, y_test):
    ...
    out = LSTM({{choice([16, 32, 64, 128])}}, activation='relu', return_sequences=True)(inputs)
    out = BatchNormalization()(out)
    out = Dropout({{uniform(0, 1)}})(out)
    out = AttentionLayer({{choice(range(10, 60, 5))}})(out)
    out = Dense({{choice([64, 128, 256])}}, activation='relu')(out)
    out = BatchNormalization()(out)
    out = Dropout({{uniform(0, 1)}})(out)
    out = Dense({{choice([8, 16, 32])}}, activation='relu')(out)
    out = BatchNormalization()(out)
    out = Dropout({{uniform(0, 1)}})(out)
    ...
    filepath = './models/'+'lstm_model-{epoch:02d}-{accuracy:.3f}-{val_accuracy:.3f}.h5'
...
if __name__ =='__main__':
    best_run, best_model = optim.minimize(model=build_model, data=data, algo=tpe.suggest, max_evals=30, trials=Trials())
    x_train, y_train, x_validation, y_validation, x_test, y_test = data()
    #best_model = load_model('./models/lstm_model-51-0.515-0.627.h5', custom_objects={'AttentionLayer':AttentionLayer})
    # train
